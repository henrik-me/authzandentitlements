...
test_scenario_read_own_tenant_account if {
	...
	object.remove(d, ["rule"]) == {"decision": "Permit", "reason": "Permit", "obligations": []}
}
...
test_scenario_read_other_tenant_account if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "TenantMismatch", "obligations": []}
}
...
test_scenario_teller_create_account if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "RoleNotAuthorized", "obligations": []}
}
...
test_scenario_teller_create_small_txn if {
	...
	object.remove(d, ["rule"]) == {"decision": "Permit", "reason": "Permit", "obligations": ["post_immediately"]}
}
...
test_scenario_teller_create_large_txn if {
	...
	object.remove(d, ["rule"]) == {"decision": "Permit", "reason": "Permit", "obligations": ["require_approval"]}
}
...
test_scenario_manager_approve_own_txn_sod if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "MakerEqualsChecker", "obligations": []}
}
...
test_scenario_manager_approve_already_approved if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "NotPending", "obligations": []}
}
...
test_scenario_teller_create_txn_no_scope if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "MissingScope", "obligations": []}
}
...
test_scenario_teller_create_txn_for_other_maker if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "SubjectNotMaker", "obligations": []}
}
...
test_scenario_unknown_action_fails_closed if {
	...
	object.remove(d, ["rule"]) == {"decision": "Deny", "reason": "UnknownAction", "obligations": []}
}
...
# ---------------------------------------------------------------------------
# CS16 — determining-rule (`rule`) coverage. The additive `rule` field names the
# determining check as "<action-short>.<Reason>" so an explanation can compare
# the deciding rule across engines (mirrors Cedar's policy-id naming).
# ---------------------------------------------------------------------------

# A permitted read carries the read.Permit rule.
test_rule_read_permit if {
	d := bank.decision with input as request(teller("user-teller1", "CONTOSO"), "bank.account.read", account("CONTOSO"), read_scope)
	d.rule == "read.Permit"
}

# A denied read (tenant mismatch) carries the read.TenantMismatch rule.
test_rule_read_tenant_mismatch if {
	d := bank.decision with input as request(teller("user-teller1", "CONTOSO"), "bank.account.read", account("FABRIKAM"), read_scope)
	d.rule == "read.TenantMismatch"
}

# A denied transaction.create (missing scope) carries the fully-qualified rule.
test_rule_transaction_create_missing_scope if {
	d := bank.decision with input as request(teller("user-teller1", "CONTOSO"), "bank.transaction.create", transaction("CONTOSO", 250, "user-teller1"), no_scopes)
	d.rule == "transaction.create.MissingScope"
}

# A permitted transaction.create carries the transaction.create.Permit rule.
test_rule_transaction_create_permit if {
	d := bank.decision with input as request(teller("user-teller1", "CONTOSO"), "bank.transaction.create", transaction("CONTOSO", 15000, "user-teller1"), txn_write_scope)
	d.rule == "transaction.create.Permit"
}

# A self-approval (SoD) denial carries the approval.MakerEqualsChecker rule.
test_rule_approval_sod if {
	d := bank.decision with input as request(manager("user-manager1", "CONTOSO"), "bank.transaction.approve", transaction_status("CONTOSO", 15000, "user-manager1", "Pending"), approvals_scope)
	d.rule == "approval.MakerEqualsChecker"
}

# The fail-closed unknown-action path carries the unknown.UnknownAction rule.
test_rule_unknown_action if {
	d := bank.decision with input as request(teller("user-teller1", "CONTOSO"), "bank.account.delete", account("CONTOSO"), read_scope)
	d.rule == "unknown.UnknownAction"
}