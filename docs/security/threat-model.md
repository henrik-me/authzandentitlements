...
| Scope/role escalation via forged claims | Claims come only from the signed token (`MapInboundClaims=false`, literal `roles`/`scope`): `AuthenticationSetup.cs:104,132-133`; roles gate specific operations at the API | Trust rests on token integrity (see Spoofing) | — |
