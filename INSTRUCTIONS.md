...
- **Bootstrap sanity check:** before claiming any CS, run `harness startup`
  ...
  ```bash
  npx -y github:henrik-me/agent-harness#v0.13.0 startup --pull-ff-only
  ```
  ...
  npx -y github:henrik-me/agent-harness#v0.13.0 sync --mode=check --cwd .        # expect: "No drift detected"
...
`npx -y github:henrik-me/agent-harness#v0.13.0 <cmd>` — there is **no `package.json`**,
...
2. Run `npx -y github:henrik-me/agent-harness#v0.13.0 startup` (session sanity + queue).
