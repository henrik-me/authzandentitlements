...
// Coarse mirror of the actor / on-behalf-of (OBO) claim contract for the edge gateway. The gateway
// is deliberately independent of Bank.Api and RE-DECLARES the Keycloak claim names it needs (like
// GatewayClaims), so it can recognise a non-human caller and a delegated call without a code
// dependency on the domain API. This is the OBO seam CS21 (break-glass / time-boxed delegation)
// reuses at the edge.
//
...
// missing or blank claim is never fabricated into a value or defaulted to a delegation. The only
// default is the safe human default (absent subject_type => "user").
public static class GatewayActorClaims
{
    ...
    // blank OBO claim is NOT a delegation.
