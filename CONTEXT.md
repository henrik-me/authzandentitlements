...
**CS46 (Keto + Topaz expansion engine adapters) complete** (Keto content PR #172, Topaz content PR #176; close-out PR follows). The final two CS26 expansion engines shipped behind the CS05 `IAuthorizationDecisionProvider` seam, completing the set (Oso de-scoped in ADR-0008 / CS47). **Keto** (`Name="keto"`, ReBAC) reuses the shared `Rebac*` seed graph over Keto's dual-port HTTP REST API (read `:4466` / write `:4467`) with an OPL namespace config (`infra/keto/`), on `oryd/keto:v26.2.0`. **Topaz** (`Name="topaz"`, full-decision) drives Topaz's OPA bundle over the **same** `infra/opa/policy` Rego via the Aserto authorizer — the "OPA standalone vs OPA-inside-Topaz" head-to-head — booting fully offline from a **local** bundle (no OCI registry push), on `ghcr.io/aserto-dev/topaz:0.33.14`. Both are opt-in **pinned** containers wired `WithExplicitStart` with **no hard `WaitFor`**, register in `AddPdp` (with explicit `Playground.razor` entries), and rely on the CS45 `ExtendedContextGuardProvider` for OBO/delegation/break-glass (no per-adapter guard), so the default `reference` path and the Docker-free build/test/`aspire run` loop are unchanged. `dotnet build` 0/0; full-solution `dotnet test` **1796/1796** (PDP 1031→1039; AppHost application-model smoke test 2/2 with both new containers). Live head-to-head validated against real pinned containers (Keto 3/3 on `RebacScenarioCatalog.Forward`; Topaz 2/2 on the full `FintechScenarioCatalog`). Independent GPT-5.5 reviews-of-record on both PRs (each R1 Needs-Fix → R2 Go — Keto caught a subject-set write fail-open, Topaz a malformed-obligations fail-open) + a Copilot review on each + plan-vs-impl **GO**. Adapter docs `docs/authz/keto-adapter.md`, `docs/authz/topaz-adapter.md`. New learnings **LRN-083..084**; **LRN-082** (`claim_area: cs46`) reconciled + flipped applied.
...
**CS50 (AppHost application-model CI smoke test) complete** (content PR #168, squash-merged 2026-07-05 as `2e27035`; close-out PR pending). Follow-up to CS48 / **LRN-078**: adds `tests/AuthzEntitlements.AppHost.Tests`, which constructs the `AuthzEntitlements.AppHost` **application model** via `Aspire.Hosting.Testing` (`CreateAsync` → `BuildAsync`, **never** `StartAsync`, so it stays Docker-free) and asserts (a) the model builds without throwing and (b) all Aspire resource names are unique case-insensitively — the exact CS48 collision class. Registered in `AuthzEntitlements.sln` so `dotnet test AuthzEntitlements.sln` (and therefore CI) runs it with **no workflow change**; pins `Aspire.Hosting.Testing 13.1.0` via CPM. Verified both directions: **2/2** pass on the current AppHost, and both fail (`DistributedApplicationException: … name 'unleash' … already exists`) when the CS48 duplicate is reintroduced. `dotnet build` 0/0; `harness lint` 23/0; independent GPT-5.5 review-of-record (Go) + Copilot (no comments) + plan-vs-impl **GO**. **LRN-078 flipped → applied;** new **LRN-079** (`dotnet sln add` rewrites the `.sln` CRLF+BOM on Windows — re-normalize to LF) + **LRN-080** (test `.cs` needs explicit `using Xunit;`).
...
**CS45 (delegation/OBO fail-closed guard across swappable PDP adapters) complete** (content PR #159, squash-merged 2026-07-05 as `fa15868`; close-out PR pending). Closes the cross-cutting **fail-OPEN** where a non-reference PDP engine selected via `Pdp:Provider` evaluated an on-behalf-of (`Subject.Actor`) / delegation (`Context.Delegation`) / break-glass (`Context.BreakGlass`) request by the human subject alone — permitting access the CS19/CS21 reference logic denies. The remedy is **capability-gated + centralized**: a marker `ISupportsExtendedAuthorizationContext` (only `reference` declares it) + a fail-closed `ExtendedContextGuardProvider` decorator that `AuthorizationDecisionProviderFactory` wraps around **every** non-capable provider, denying any extended-context request with the distinct `ExtendedContextUnsupported` reason (deliberately free of the "unavailable" substring so `PlaygroundFanoutService` treats it as a genuine deny, not an engine outage) — never permits, never throws, transparent otherwise. It covers the enforced `PdpDecisionService` **and** the factory-resolved shadow/what-if/playground paths, so an engine swap can no longer silently weaken OBO/delegation/break-glass. The CS26 Cerbos in-adapter guard was **removed** in favour of this single authoritative seam (Decision #6); every future engine (CS46 Keto/Topaz) inherits the guard automatically. Contract docs (`pdp-contract.md`, `adding-an-engine-adapter.md`, `cerbos-adapter.md`) document the boundary + opt-in. `dotnet build` 0/0; PDP 922/922; full solution 1677/1677; independent GPT-5.5 **security review** (adversarial fail-open hunt) **Go** + Copilot clean + plan-vs-impl **GO**. **LRN-075 flipped → applied.** Un-staled onto the harness v0.17.0 upgrade (#157) mid-flight.

**CS47 (Oso disposition — de-scope from the expansion-engine set) complete** (content PR #170, admin-squash-merged 2026-07-05 as `2cc9f7e`; close-out PR pending). Docs-only disposition formally **de-scoping Oso** from the CS26 expansion-engine adapter set: adds **ADR 0008** (+ `docs/adr/README.md` index) and marks Oso **evaluated → de-scoped** across the comparison matrix, market survey, and the policy-and-decision-engines survey (superseding the old "local/self-hosted PDP binary" wording), plus Oso-only consistency fixes in the managed-vs-selfhost TCO doc. **De-scope rationale (evidence re-verified 2026-07-05 against primary sources):** Oso has **no in-process .NET/Polar library** (`nuget.org/packages/Oso` → 404) and **no self-hostable production server** — its only self-hostable artifact is a **development-only** dev-server (pinnable to versioned tags + a native binary, but vendor-scoped to dev/testing), with production on the paid, proprietary managed **Oso Cloud**; this is off the ADR-0007 self-host-first posture. An independent GPT-5.5 rubber-duck review **caught + corrected** an inherited CS26 factual error — the dev-server is **not** "latest-only/unpinnable" — across the ADR + eval docs before merge (**LRN-081**). Re-eval trigger: revisit only if Oso ships a maintained in-process .NET/Polar library **or** a self-hostable, production-supported server. No adapter code / NuGet pin / container. `harness lint` 23/0; independent GPT-5.5 review-of-record (Go) + Copilot + plan-vs-impl **GO**. The expansion set now reads **Cerbos + SpiceDB shipped (CS26), Keto + Topaz planned (CS46), Oso de-scoped (CS47)**.

**CS53 (codify .NET/Windows build-tooling conventions) complete** (content PR #182, squash-merged 2026-07-05 as `ec0bd5c`; plan-vs-impl scope fix PR #183 as `d6549f3`; close-out PR pending). Documentation-only harvest of **LRN-079/080** into the `CONVENTIONS.md` `conventions.project` "Language + build" block: (a) `dotnet sln add` on Windows rewrites `AuthzEntitlements.sln` with CRLF + a UTF-8 BOM (fails the `text-encoding` gate) → after any `dotnet sln` edit re-normalize the `.sln` to LF / no-BOM before committing (`remove` handled conservatively); (b) every xUnit test `.cs` needs an explicit `using Xunit;` despite `ImplicitUsings` (else `CS0246`). No code / `.csproj` / `.sln` change — the shipped solution + test suites already comply. `harness lint` 23/0; independent GPT-5.5 review-of-record (Go, R1→R2 after a Copilot `$sln` self-contained-snippet suggestion) + Copilot + plan-vs-impl **GO** (R1 NEEDS-FIX caught an over-claim that `dotnet sln remove` was *observed* to rewrite CRLF+BOM → scoped to `add` per LRN-079 in PR #183 → R2 GO). **LRN-079 + LRN-080 flipped → applied.**

**CS54 (codify out-of-process PDP adapter safety conventions) complete** (content PR #187, admin-squash-merged 2026-07-05 as `a909104`; close-out PR pending). Documentation-only harvest of **LRN-072/073/074/076** (all `source_cs: CS26`, `claim_area: pdp-adapters`) into `docs/authz/pdp-contract.md` § "Out-of-process engine adapter safety" — the four out-of-process / full-decision engine-adapter safety patterns, each **scoped by transport/role** and cited by file+concept (never line number): (a) the h2c `Http2UnencryptedSupport` `AppContext` switch set **before the first** `SocketsHttpHandler`/`GrpcChannel` (cleartext-gRPC-only) + `https://` fail-closed rejection; (b) lowercase gRPC metadata / `CallCredentials` keys (**any** gRPC adapter, cleartext or TLS); (c) the full-decision fail-closed response-mapping checklist (unknown obligation → deny, no-output → `ProviderUnavailable`, ambiguous → deny); (d) the env-gated `<ENGINE>_TEST_ENDPOINT` soft-skip integration test (**every** out-of-process adapter) — plus a discoverable pointer in the `CONVENTIONS.md` `conventions.project` block. No code change; `dotnet build` 0/0, `dotnet test` **1796** green, `harness lint` 23/0. Reviewed across **5 rounds** — an independent GPT-5.5 rubber-duck (R3 **NEEDS-FIX** caught a transport-scoping regression that mislabelled the metadata-casing rule "cleartext-only") + Copilot (caught stale "future Keto/Topaz", an offline-casing-test overclaim, and over-generalized applicability) + plan-vs-impl **GO**. **LRN-072/073/074/076 flipped → applied;** new **LRN-086** (Topaz adds mixed-case gRPC metadata keys — verify client normalization or lowercase; `claim_area: pdp-adapters`).

**Next claimable:** with **CS11, CS13, CS14, CS15, CS16, CS17, CS18, CS19, CS20, CS21, CS22, CS23, CS24, CS25, CS26, CS28, CS29, CS30, CS31, CS32, CS33, CS34, CS45, CS48, CS50, CS53, and CS54 DONE**, **CS27 was rescoped 2026-07-04** and split: app Azure deploy stays **CS27**, full OpenMeter (local) is **CS43** (CS10+CS12), Azure OpenMeter is **CS44** (CS25+CS27+CS43). **⛔ CS27, CS43, and CS44 remain HELD** — do NOT claim without explicit user confirmation: their precondition #1 (documented local validation) is now satisfied by CS48, but lifting still needs a maintainer go-ahead + observability-warrant decision (cloud/Azure deploy is out of scope at this time; CS48's assessment says OpenMeter not warranted yet). Each carries a `## Hold / claim gate` + an `open` LEARNINGS backstop (LRN-069/070/071, `claim_area: cs27/cs43/cs44`) and is registered HIGH-RISK in `harness.config.json`. Also ready: the CS26 follow-ups — **CS45** (OBO/delegation/break-glass adapter guard) is now **DONE**, which unblocks **CS46** (Keto+Topaz adapters — depends on CS45; claim first); **CS47** (Oso de-scope disposition) is now **DONE** (PR #170). **LRN-078** is now **applied** — the recommended AppHost application-model CI smoke test shipped as **CS50** (DONE, PR #168). Check each file's `**Depends on:**` and the WORKBOARD before claiming. `harness lint` is green and the remaining CSs carry an independent GPT-5.5 `## Plan review` attestation.
