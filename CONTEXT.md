...
**Next claimable:** with **CS11, CS13, CS14, CS15, CS16, CS17, CS18, CS19, CS20, CS21, CS22, CS23, CS24, CS25, CS26, CS28, CS29, CS30, CS31, CS32, CS33, and CS34 DONE**, **CS25-done newly unblocks CS27** (Azure deployment of the app via azd → ACA — needs CS10+CS12+CS14+CS15+CS25, all done; the CS25 TCO doc's Azure cloud-move section is the direct input). **CS27 was rescoped 2026-07-04**: the original "full OpenMeter + Azure deploy" was split so the app cloud-deploy is decoupled from metering infra — full OpenMeter metering moved to **CS43** (local — CS10+CS12) and Azure deployment of OpenMeter to **CS44** (CS25+CS27+CS43). **⛔ CS27, CS43, and CS44 are HELD (2026-07-04)** — do NOT claim without explicit user confirmation: gated pending detailed local validation of the current stack, demo/lab observability justification, and a maintainer go-ahead (cloud/Azure deploy is out of scope at this time). Each carries a `## Hold / claim gate` section + an `open` LEARNINGS backstop (LRN-069/070/071, `claim_area: cs27/cs43/cs44`; surfaces at the weekly `harness harvest` now and at `harness claim` once ≥14 days stale), and each is registered HIGH-RISK in `harness.config.json`. **CS48 (local stack validation + demo/lab readiness) is filed as the prerequisite that satisfies those holds' precondition #1** — validate the current stack locally (build + full test suite + `aspire run` smoke) and document it; lifting the holds additionally needs the user's explicit go-ahead + an observability-warrant decision. Also ready: the newly-planned CS26 follow-ups — **CS45** (cross-cutting OBO/delegation/break-glass adapter guard — claim first), **CS46** (Keto+Topaz adapters — depends on CS45), and **CS47** (Oso de-scope disposition). The **CS28h-harvest queue is complete** (CS29/CS30/CS31/CS32/CS33 done). Check each file's `**Depends on:**` and the WORKBOARD (some are actively owned) before claiming. `harness lint` is green and the remaining CSs carry an independent GPT-5.5 `## Plan review` attestation.
