...
**Next claimable:** with **CS11, CS13, CS14, CS15, CS16, CS17, CS18, CS19, CS20, CS21, CS22, CS23, CS24, CS25, CS28, CS29, CS30, CS31, CS32, CS33, and CS34 DONE**, **CS25-done newly unblocks CS27** (Azure deployment of the app via azd → ACA — needs CS10+CS12+CS14+CS15+CS25, all done; the CS25 TCO doc's Azure cloud-move section is the direct input). **CS27 was rescoped 2026-07-04**: the original "full OpenMeter + Azure deploy" was split so the app cloud-deploy is decoupled from metering infra — full OpenMeter metering moved to **CS43** (local — CS10+CS12, ready now) and Azure deployment of OpenMeter to **CS44** (CS25+CS27+CS43). Also ready: **CS26** (expansion engines — CS05+CS15). The **CS28h-harvest queue is complete** (CS29/CS30/CS31/CS32/CS33 done). Check each file's `**Depends on:**` and the WORKBOARD (some are actively owned) before claiming. `harness lint` is green and the remaining CSs carry an independent GPT-5.5 `## Plan review` attestation.
