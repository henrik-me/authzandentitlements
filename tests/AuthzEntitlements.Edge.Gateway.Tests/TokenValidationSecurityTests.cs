...
// CS18 security hardening (gateway mirror of Bank.Api's TokenValidationSecurityTests).
// The edge must reject exactly the same bad tokens the downstream API does. Part A asserts
// the hardened TokenValidationParameters (tightened ClockSkew, required expiration, required
// signatures) plus regression guards; Part B crafts tokens offline (no Keycloak/network) and
// drives them through the REAL configured parameters to prove expired/forged/wrong-issuer/
// wrong-audience/non-expiring tokens are rejected. See docs/security/threat-model.md
// (Spoofing/Tampering).
public sealed class TokenValidationSecurityTests
{
    ...
    {
        Assert.Equal(TimeSpan.FromSeconds(30), GatewayAuthenticationSetup.MaxClockSkew);
        Assert.Equal(GatewayAuthenticationSetup.MaxClockSkew, BuildParameters().ClockSkew);
