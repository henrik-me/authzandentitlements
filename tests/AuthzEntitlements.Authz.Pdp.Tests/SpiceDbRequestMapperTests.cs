...
// Pure mapping tests for the SpiceDB adapter: the AuthZEN AccessRequest -> SpiceDB permission-check
// translation and its fail-closed paths. No client or server — SpiceDbRequestMapper is a pure
// function, which is exactly why the provider factors it out. Mirrors OpenFgaProviderMappingTests so
// the two ReBAC adapters demonstrably map the same request to the same relationship question.
public sealed class SpiceDbRequestMapperTests
{
    ...
    public void AccountRead_MapsTo_CanView()
    {
        ...
        var mapped = SpiceDbRequestMapper.TryMap(request, out var check, out _);
    ...
    public void UnknownAction_FailsClosed_UnknownAction()
    {
        ...
        var mapped = SpiceDbRequestMapper.TryMap(request, out _, out var denial);
    ...
    public void NonAccountResource_FailsClosed_UnsupportedResourceType()
    {
        ...
        // schema, so it must deny cleanly rather than build a "transaction:..." object SpiceDB rejects.
    ...
    public void BlankResourceId_FailsClosed(string resourceId)
    {
        ...
        Assert.False(SpiceDbRequestMapper.TryMap(request, out _, out var denial));
        Assert.Equal(RebacReasonCodes.MissingResourceId, denial.Reasons[0].Code);
    }
