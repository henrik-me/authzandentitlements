...
// KetoProvider's permit/deny/fail-closed behaviour through the IKetoCheckClient seam (LRN-038),
// asserted fully OFFLINE via a test double — the Keto counterpart to SpiceDbProviderTests /
// OpenFgaProviderSeamTests. Also covers registration + selection through the CS05 seam (AddPdp), so the
// "keto" provider is factory-selectable by name, coexists with the reference engine, and never touches
// a live server.
public sealed class KetoProviderTests
{
    private static KetoProvider Provider(IKetoCheckClient client) =>
        new(client, NullLogger<KetoProvider>.Instance);
    ...
    {
        var decision = Provider(FakeKetoCheckClient.Allowing())
    ...
    {
        var decision = Provider(FakeKetoCheckClient.Denying())
    ...
    public void Evaluate_ForwardsTheMappedCheck_ToTheSeam()
    {
        ...
        var fake = FakeKetoCheckClient.Allowing();
    ...
    public void Evaluate_WhenSeamThrows_FailsClosed_Deny_EngineUnavailable()
    {
        ...
        var decision = Provider(FakeKetoCheckClient.Throwing(new InvalidOperationException("engine down")))
            .Evaluate(AccountRead("carol", "personal-carol"));
    ...
    {
        // A fail-closed boundary deny (unknown action) must short-circuit BEFORE any check: the seam is
        // never consulted, so a mapper-level deny can never be turned into an accidental engine permit —
        // proven by an always-ALLOW double that records zero calls.
        var fake = FakeKetoCheckClient.Allowing();
        var request = new AccessRequest(
            new Subject("user", "carol", []),
    ...
    private static ServiceProvider Build(string provider, string readEndpoint = "", string writeEndpoint = "")
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Pdp:Provider"] = provider,
                ["Pdp:Keto:ReadEndpoint"] = readEndpoint,
                ["Pdp:Keto:WriteEndpoint"] = writeEndpoint,
            })
            .Build();
    ...
    [Fact]
    public void AddPdp_RegistersKetoProvider_AmongProviders()
    {
        using var provider = Build("reference");

        var names = provider.GetServices<IAuthorizationDecisionProvider>().Select(p => p.Name).ToList();

        Assert.Contains("keto", names);
        Assert.Contains("reference", names);
    }

    [Fact]
    public void AddPdp_SelectsKeto_WhenConfigured_CaseInsensitively()
    {
        using var provider = Build("KETO");

        var factory = provider.GetRequiredService<AuthorizationDecisionProviderFactory>();

        // CS45: keto does not declare ISupportsExtendedAuthorizationContext, so the factory wraps it in
        // the fail-closed ExtendedContextGuardProvider. Selection is unchanged (Name still "keto"); the
        // resolved instance is the guard whose Inner is the concrete Keto adapter.
        var active = factory.GetActiveProvider();
        Assert.Equal("keto", active.Name);
        var guard = Assert.IsType<ExtendedContextGuardProvider>(active);
        Assert.IsType<KetoProvider>(guard.Inner);
    }

    [Fact]
    public void KetoOptions_BindFromPdpKetoSection()
    {
        using var provider = Build("keto", readEndpoint: "http://localhost:4466", writeEndpoint: "http://localhost:4467");

        var options = provider.GetRequiredService<IOptions<KetoOptions>>().Value;

        Assert.Equal("http://localhost:4466", options.ReadEndpoint);
        Assert.Equal("http://localhost:4467", options.WriteEndpoint);
        Assert.Equal("Pdp:Keto", KetoOptions.SectionName);
    }

    [Fact]
    public void Registration_DoesNotRequireServer_BlankEndpointsAreFine()
    {
        // Building the container and resolving the provider must succeed with no Keto server and empty
        // endpoints — the live REST clients are built lazily only on first actual check.
        using var provider = Build("keto", readEndpoint: "", writeEndpoint: "");

        var keto = provider.GetServices<IAuthorizationDecisionProvider>().Single(p => p.Name == "keto");

        Assert.NotNull(keto);
        Assert.NotNull(provider.GetRequiredService<KetoCheckService>());
    }

    [Fact]
    public void Evaluate_FailsClosed_WhenEngineUnavailable()
    {
        // With blank endpoints the service throws when the provider issues its check; Evaluate must DENY
        // (fail closed) with a stable reason, never throw a 500 through /api/authz/evaluate.
        using var provider = Build("keto", readEndpoint: "", writeEndpoint: "");
        var keto = provider.GetServices<IAuthorizationDecisionProvider>().Single(p => p.Name == "keto");

        var decision = keto.Evaluate(AccountRead("carol", "personal-carol"));
    ...
    [Fact]
    public void UnknownProvider_StillFailsClosed_ListingKeto()
    {
        ...
        Assert.Contains("keto", ex.Message);
    }
}
