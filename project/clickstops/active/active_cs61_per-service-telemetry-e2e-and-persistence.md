...
|---|---|---|---|
| Strengthen `TelemetryArrivalE2ETests` — drive `/alive` (assert success) to all 7 services; poll until every service pushes a fresh `http_server` sample (timestamp advances past baseline) | done | omni-ae | agent-id=omni-ae \| role=test \| report-status=complete \| learnings=0 — Decision 1; **passes live** clean slate (1m7s) + retained volume (baseline non-zero → newer ts) |
...
- **Per-service e2e:** `TelemetryArrivalE2ETests` now drives `/alive` to all 7 project services (asserting each `/alive` succeeds) and polls per-service until **every** service pushes a **fresh** `http_server_request_duration_seconds_count` `job` sample this run — it compares `max by (job)(timestamp(...))` against a pre-traffic baseline and requires the latest-sample timestamp to advance (replacing the old aggregate `> 0` + `>= 2`). Verified live: passes on a clean slate (1m7s) **and** on a retained volume (baseline non-zero → final timestamps ~70s newer for all 7).
- **Delivery signal evolved during content-PR review (#217).** R1 shipped a per-`job` `count > 0` poll; GPT-5.5 R2 flagged that a stale series satisfies `> 0` without proving this-run delivery, and a value-change/increase check is unsafe (per-process counter **resets** can lower the value, and a `sum by (job)` value can change merely because a **stale series ages out** of Prometheus's 5m lookback mid-poll — a broken service could falsely pass). Final design compares **sample timestamps** (`max by (job)(timestamp(...))`, require strictly newer than baseline), which is immune to both resets and aging. Verified live on a retained volume.
